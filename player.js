...
sampleplayer.CastPlayer.prototype.createHost = function (url, mediaElement, mediaInformation) {
  var options = {};
  options.url = url;
  options.mediaElement = mediaElement;
  if (typeof mediaInformation.customData === 'object') {
    var customData = mediaInformation.customData;
    if (customData.licenseUrl) {
      options.licenseUrl = customData.licenseUrl;
    }
    if (customData.protectionSystem) {
      options.protectionSystem = customData.protectionSystem;
    }
    if (customData.licenseCustomData) {
      options.licenseCustomData = customData.licenseCustomData;
    }
    options.initialBandwidth = customData.initialBandwidth;
  }
  this.log_(options);
  var host = new cast.player.api.Host(options);
  return host;
};

sampleplayer.CastPlayer.prototype.getHlsSegmentFormat = function (mediaInformation) {
  if (typeof mediaInformation.customData === 'object') {
    var customData = mediaInformation.customData;
    if (customData.hlsSegmentFormat) {
      switch (customData.hlsSegmentFormat) {
        case 'MPEG_AUDIO_ES':
...
sampleplayer.CastPlayer.prototype.getIsLive = function (mediaInformation) {
  if (typeof mediaInformation.customData === 'object') {
    var customData = mediaInformation.customData;
    if (typeof customData.isLive === 'boolean' && customData.isLive) {
      return true;
    }
  }
  return false;
...
sampleplayer.CastPlayer.prototype.preloadVideo_ = function (mediaInformation) {
  ...
  var host = this.createHost(url, self.mediaElement_, mediaInformation);
  ...
  var hlsSegmentFormat = this.getHlsSegmentFormat(mediaInformation);
  var isLive = this.getIsLive(mediaInformation);
  if (protocolFunc === cast.player.api.CreateHlsStreamingProtocol && hlsSegmentFormat !== cast.player.api.HlsSegmentFormat.MPEG2_TS) {
    if (isLive) {
      self.preloadPlayer_.preload(protocolFunc(host, hlsSegmentFormat), Infinity);
    } else {
      self.preloadPlayer_.preload(protocolFunc(host, hlsSegmentFormat));
    }
  } else {
    if (isLive) {
      self.preloadPlayer_.preload(protocolFunc(host), Infinity);
    } else {
...
sampleplayer.CastPlayer.prototype.load = function (info) {
  this.log_('Loads the given data');
  this.log_(info);
...
sampleplayer.CastPlayer.prototype.loadVideo_ = function (info) {
  ...
  protocolFunc = sampleplayer.getProtocolFunction_(info.message.media);
  ...
  if (!protocolFunc) {
    ...
    if (!this.preloadPlayer_ || (this.preloadPlayer_.getHost &&
      ...
      this.log_('Regular video load1');
      /*var host = new cast.player.api.Host({
        'url': url,
        'mediaElement': this.mediaElement_
      });*/
      var host = this.createHost(url, this.mediaElement_, info.message.media);
      host.onError = loadErrorCallback;
      this.player_ = new cast.player.api.Player(host);
      var hlsSegmentFormat = this.getHlsSegmentFormat(info.message.media);
      var isLive = this.getIsLive(info.message.media);
      if (protocolFunc === cast.player.api.CreateHlsStreamingProtocol && hlsSegmentFormat !== cast.player.api.HlsSegmentFormat.MPEG2_TS) {
        if (isLive) {
          this.player_.load(protocolFunc(host, hlsSegmentFormat), Infinity);
        } else {
          this.player_.load(protocolFunc(host, hlsSegmentFormat));
        }
      } else {
        if (isLive) {
          this.player_.load(protocolFunc(host), Infinity);
        } else {
